...
<link rel="stylesheet" type="text/css" href="__STYLE__article.css">
<link rel="stylesheet" type="text/css" href="__STYLE__spectrum.css">
...
<script type="text/javascript" src="__PUBLIC__js/article_edit.js"></script>
<script type="text/javascript" src="__PUBLIC__js/edit.js"></script>
<script type="text/javascript" src="__PUBLIC__js/spectrum.js"></script>
<script type="text/javascript">
	...
	$(function(){		
		if($('#thumbsrc').val() !=''){
			...
			$('#pic_show').show().html('<img src="__ROOT__/'+thumbsrc+'" alt="缩略图"/>');
		}
		$("#picUpload").wrap("<form id='myupload' action='{:U('Public/fileUpload', array('tb' => 2))}' method='post' enctype='multipart/form-data'></form>");
			...
			$('#myupload').ajaxSubmit({
				...
				success:function(data){
					...
					if(typeof data == 'string'){
						$('#thumbsrc').val(data);
						$('#pic_show').show().html('<img src="__ROOT__/'+data+'" alt="缩略图"/>');
						$.messager.show({
							title:'恭喜：',
							msg:'图片上传成功',
							timeout:4000,
...
<body>
<h2><a href="{:U('article/index')}">返回上一级</a>文档管理 <if condition="$position">&gt; {$position}</if></h2>
...
<dl class="edit">
	...
	<dd>
		属　　性：
		<foreach name="attrArray" item="val">
			<label><input type="checkbox" name="attr[]" value="{$key}"/> {$val} </label> 
		</foreach>
...
<div id="msg"></div>
<div id="win">
	<foreach name="arrDir" item="vo" key="k" > 
		 <h3>{$k}</h3>
		 <ul>
		 	<foreach name="vo" item='val'>
		 		<li><img src="__ROOT__/Uploads/{$k}/{$val}" title="/Uploads/{$k}/{$val}"/></li>
		 	</foreach>
		 </ul>
	 </foreach>
